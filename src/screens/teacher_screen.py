...
from src.components.dialog_subject_create import create_subject_dialog
from src.components.subject_card import subject_card
from src.components.dialog_share_subject import share_subject_dialog
from src.components.dialog_add_photo import add_photos_dialog
from components.dialog_attendance_result import attendance_result_dialog
from src.pipelines.face_recognition import predict_attendance
...
def teacher_screen():
    ...
    if 'teacher_data' in st.session_state:
        teacher_dashboard()
    elif 'teacher_login_type' not in st.session_state or st.session_state.teacher_login_type=="register":
...
def teacher_screen_login():
    ...
    st.space()
    st.space()
    teacher_username= st.text_input("Enter Username" , placeholder="@krish")
...
def teacher_register(teacher_username, teacher_name, teacher_pass, teacher_pass_confirm):
    ...
    try:
        ...
        return True, "Succeessfuly Created Account! Please Login now "
...
def teacher_screen_register():
    ...
    st.space()
    teacher_username = st.text_input("Enter Username" , placeholder="@krish")
    teacher_name = st.text_input("Enter Name" , placeholder="Krish Gill")
    teacher_pass= st.text_input("Enter Password" , type='password', placeholder="Enter your password")
    ...
    with btnc1:
        if st.button("Register now" , icon=":material/passkey:" , shortcut="control+enter" , width="stretch"):
            success , msg = teacher_register(teacher_username, teacher_name, teacher_pass, teacher_pass_confirm)
            if success:
                st.success(msg)
                import time 
                time.sleep(2)
